...
INSERT INTO `xxl_job_group`(`id`, `app_name`, `title`, `address_type`, `address_list`, `update_time`)
    VALUES (1, 'xxl-job-executor-sample', '通用执行器Sample', 0, NULL, now()),
           ...
           (3, 'springmall-executor', 'springMall 订单服务', 0, NULL, now());
...
VALUES (1, 1, '示例任务01', now(), now(), 'XXL', '', 'CRON', '0 0 0 * * ? *',
    ...
    "prompt": "你是一个出游助手，擅长做旅游规划"
        ...
        now(), ''),
       (5, 3, '订单归档任务', now(), now(), 'system', '', 'CRON', '0 0 2 * * ?',
        'DO_NOTHING', 'FIRST', 'orderArchiveJobHandler', '', 'SERIAL_EXECUTION', 0, 0, 'BEAN', '', 'GLUE代码初始化',
        now(), '');

-- [安全警告] 默认管理员密码为 123456（SHA-256）。首次登录后请立即在"用户管理"界面修改密码。
